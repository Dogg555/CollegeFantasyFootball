(function initLeagueDashboardRuntime(root) {
  ...
  const { dashboardViewModel, saveCache, loadCache, cacheScope } = helpers;
  ...
  function renderUnavailable(message) {
    const hub = ensureHub();
    if (!hub) return;
    hub.dataset.state = 'unavailable';
    setText('league-dashboard-state', message || 'Dashboard data is temporarily unavailable.');
  }
  ...
  async function refreshDashboard({ allowCached = true } = {}) {
    ...
    if (!auth?.token || !auth?.email || !league?.id || String(auth.token).startsWith('local-demo-')) return;
    const scope = cacheScope(auth.email, league.id);
    if (activeRequest && lastScope === scope) return activeRequest;
    lastScope = scope;
    ...
    activeRequest = (async () => {
      ...
      try {
        ...
        const cached = allowCached ? loadCache(root.localStorage, auth.email, league.id) : null;
        if (cached) {
          renderDashboard(cached);
          return cached;
        }
  ...
  root.addEventListener('hashchange', () => {
    if ((root.location.hash || '').replace('#', '') === 'overview') refreshDashboard();
  });
