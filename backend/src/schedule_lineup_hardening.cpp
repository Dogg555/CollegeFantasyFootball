...
std::string canonicalEmail(std::string value) {
    return cff::schedule_lineup_lifecycle::canonicalEmail(std::move(value));
}
...
std::string isoNow() {
    return isoTimestamp(std::chrono::system_clock::now());
}
...
int requestWeek(const drogon::HttpRequestPtr &request, int fallback = 1) {
    if (const auto body = request->getJsonObject(); body && body->isObject() && body->isMember("week")) {
        return positiveInt((*body)["week"], fallback);
    }
...
bool parseLineupPath(const std::string &path,
    ...
    try { week = std::max(1, std::stoi(rawWeek)); } catch (...) { return false; }
    return true;
}
...
bool parseRosterSlotPath(const std::string &path,
    ...
    if (markerAt == std::string::npos) return false;
    leagueId = path.substr(prefix.size(), markerAt - prefix.size());
    const auto playerStart = markerAt + marker.size();
    playerId = path.substr(playerStart, path.size() - playerStart - suffix.size());
    return !leagueId.empty() && !playerId.empty() && playerId.find('/') == std::string::npos;
}

#ifdef CFF_HAS_POSTGRES
#include "schedule_lineup_hardening_db.inc"
#include "schedule_lineup_hardening_payload.inc"
#include "schedule_lineup_hardening_mutations.inc"
#endif

#include "schedule_lineup_hardening_advice.inc"
...
drogon::HttpResponsePtr prepareLineupsForScoring(const std::string &leagueId,
    ...
    return prepareLineupsForScoringInternal(
        leagueId, canonicalEmail(accountEmail), season, week);
#else
    (void)leagueId;
    (void)accountEmail;
    (void)season;
