...
- **Persist for your user:**
  ```powershell
  setx PATH "$($env:PATH);C:\path\to\tool\bin"
  ```
  (Re-open terminals to pick up changes.)
- **GUI:** Settings → System → About → Advanced system settings → Environment Variables → edit `Path`.
